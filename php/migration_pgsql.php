...
/**
 * migration_pgsql.php — Migration runner for PostgreSQL
 *
 * Usage:
 *   php migration_pgsql.php [schema.sql]              — diff migrations/ vs schema, generate + apply new migration
 *   php migration_pgsql.php [schema.sql] --status     — show which migrations have been applied
 *   php migration_pgsql.php [schema.sql] --apply-only — apply pending migration files without diffing
 *
...
{
    colorLine("cyan", "\n── Diffing migrations/ vs " . basename($config['schema_file']) . " ─────────");
    ...
    // ── New tables (in schema_pgsql.sql, not in replayed state) ──
    ...
    // ── Removed tables (in replayed state, not in schema_pgsql.sql) ──
        ...
        foreach ($schemaCols as $col => $schemaDef) {
            ...
            if ($schemaDef['def'] !== $replayedDef['def']) {
                ...
                if (confirm("    Apply change?")) {
                    $statements[] = buildAlterColumn($table, $col, $schemaDef);
                }
            }
        }
    }
    ...
    if (empty($statements)) {
        colorLine("green", "\nNo changes detected — schema_pgsql.sql matches migrations.");
        return;
    }
...
/**
 * Parse all CREATE TABLE statements from a PostgreSQL SQL string.
 * Also replays ALTER TABLE ADD/DROP/ALTER/RENAME COLUMN so migration files
...
function parseSchemaFile(string $sql): array
{
    ...
    // ── CREATE TABLE (handles both quoted and unquoted table names) ──
    preg_match_all(
    ...
    foreach ($alters as $a) {
        ...
        // ALTER COLUMN ... TYPE (PostgreSQL MODIFY equivalent)
        elseif (preg_match('/^ALTER\s+COLUMN\s+"?(\w+)"?\s+TYPE\s+(.+)$/i', $clause, $m)) {
            $col = $m[1];
            $def = preg_replace('/\s+/', ' ', rtrim(trim($m[2]), ', '));
            if (isset($tables[$table][$col])) {
                $tables[$table][$col] = ['def' => $def];
            }
        }
        // RENAME COLUMN
...
function parseColumnDefs(string $body): array
{
    ...
    foreach ($lines as $line) {
        $line = trim($line, " \t,");
        if ($line === '') continue;
        if (preg_match('/^(PRIMARY\s+KEY|UNIQUE|CONSTRAINT|FOREIGN)/i', $line)) continue;
...
/**
 * PostgreSQL uses ALTER COLUMN ... TYPE instead of MySQL's MODIFY COLUMN.
 * Note: NULL/NOT NULL constraints and DEFAULT values require separate clauses.
 * This emits multiple ALTER COLUMN statements as needed.
 */
function buildAlterColumn(string $table, string $col, array $def): string
{
    $raw = rtrim($def['def'], ', ');

    $statements = [];

    // Extract type (everything before NULL/NOT NULL/DEFAULT keywords)
    if (preg_match('/^(.+?)(?:\s+(NOT\s+NULL|NULL)\b|\s+DEFAULT\b|$)/i', $raw, $tm)) {
        $type = trim($tm[1]);
        $statements[] = "ALTER TABLE \"{$table}\" ALTER COLUMN \"{$col}\" TYPE {$type};";
    }

    // NULL / NOT NULL
    if (preg_match('/\b(NOT\s+NULL|NULL)\b/i', $raw, $nm)) {
        $nullClause = strtoupper(preg_replace('/\s+/', ' ', trim($nm[1])));
        if ($nullClause === 'NOT NULL') {
            $statements[] = "ALTER TABLE \"{$table}\" ALTER COLUMN \"{$col}\" SET NOT NULL;";
        } else {
            $statements[] = "ALTER TABLE \"{$table}\" ALTER COLUMN \"{$col}\" DROP NOT NULL;";
        }
    }

    // DEFAULT
    if (preg_match('/\bDEFAULT\s+(.+)/i', $raw, $dm)) {
        $default = trim($dm[1]);
        $statements[] = "ALTER TABLE \"{$table}\" ALTER COLUMN \"{$col}\" SET DEFAULT {$default};";
    }
 ...
 * Heuristic: same base type family (INT, VARCHAR, TEXT, etc.)
 */
 ...
 *   php migration_pgsql.php              → schema.sql
 *   php migration_pgsql.php my.sql       → my.sql
 *   php migration_pgsql.php my.sql --status
 */
