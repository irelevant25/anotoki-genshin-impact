...
// GET single
$app->get('/api/characters-builds-sub-stats/{id}', function (Request $request, Response $response, array $args) {
...
// PUT update
$app->put('/api/characters-builds-sub-stats/{id}', function (Request $request, Response $response, array $args) {
    $user = $request->getAttribute('user');
    $pdo  = genshinDb();
...
// DELETE
$app->delete('/api/characters-builds-sub-stats/{id}', function (Request $request, Response $response, array $args) {
    $pdo = genshinDb();
    if (!DbQuery::from($pdo, 'characters_builds_sub_stats')->find(['id' => $args['id'], 'deleted' => false])) {
        return respondJson($response, ['error' => 'Not found'], 404);
    }
