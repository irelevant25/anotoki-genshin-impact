...
// GET all
$app->get('/api/affiliations', function (Request $request, Response $response) {
    $items = DbQuery::from(genshinDb(), 'affiliations')
...
$app->post('/api/affiliations', function (Request $request, Response $response) {
    ...
    $result = DbQuery::from($pdo, 'affiliations')
        ->includeExternal('created_by', usersDb(), 'users', ['id', 'username'])
        ->find(['id' => $id]);
    return respondJson($response, $result, 201);
})->add(validateRequest(Affiliation::class))->add(requireRole(...ROLES_CONTENT))->add(requireAuth());
...
$app->put('/api/affiliations/{id:[0-9]+}', function (Request $request, Response $response, array $args) {
    ...
    DbQuery::update($pdo, 'affiliations', [
        ...Affiliation::partialToDbArray($request->getParsedBody()),
        'updated_by' => $user['id'],
    ], $args['id']);
    $result = DbQuery::from($pdo, 'affiliations')
        ->includeExternal('created_by', usersDb(), 'users', ['id', 'username'])
...
})->add(validateRequest(Affiliation::class, true))->add(requireRole(...ROLES_CONTENT))->add(requireAuth());
...
$app->delete('/api/affiliations/{id:[0-9]+}', function (Request $request, Response $response, array $args) {
    ...
    DbQuery::update($pdo, 'affiliations', ['deleted' => true], $args['id']);
