...
$app->get('/api/materials/{id:[0-9]+}/usage', function (Request $request, Response $response, array $args) {
    ...
    $charactersAscension = $query(
        'SELECT c.id, c.name, c.icon_name, c.rarity, c.element, SUM(cost.quantity) AS quantity
          ...
          GROUP BY c.id, c.name, c.icon_name, c.rarity, c.element
          ORDER BY c.name'
    );
    ...
    $weaponsAscension = $query(
        'SELECT w.id, w.name, w.icon_name, w.rarity, w.type, SUM(cost.quantity) AS quantity
          ...
          WHERE cost.material_id = ? AND cost.deleted = FALSE
          GROUP BY w.id, w.name, w.icon_name, w.rarity, w.type
          ORDER BY w.name'
    );

    $weaponsRefinement = $query(
        'SELECT w.id, w.name, w.icon_name, w.rarity, w.type, SUM(r.quantity) AS quantity
