...
// GET single
$app->get('/api/characters-builds-artifacts/{id}', function (Request $request, Response $response, array $args) {
...
})->add(validateRequest(CharacterBuildArtifact::class))->add(requireRole('ADMIN', 'EDITOR'))->add(requireAuth());

// PUT update
$app->put('/api/characters-builds-artifacts/{id}', function (Request $request, Response $response, array $args) {
    $user = $request->getAttribute('user');
    $pdo  = genshinDb();
...
})->add(validateRequest(CharacterBuildArtifact::class, true))->add(requireRole('ADMIN', 'EDITOR'))->add(requireAuth());

// DELETE
$app->delete('/api/characters-builds-artifacts/{id}', function (Request $request, Response $response, array $args) {
    $pdo = genshinDb();
    if (!DbQuery::from($pdo, 'characters_builds_artifacts')->find(['id' => $args['id'], 'deleted' => false])) {
        return respondJson($response, ['error' => 'Not found'], 404);
    }
